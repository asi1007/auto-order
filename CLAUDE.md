...
**候補があるのに一意に決まらないときは、そのグループを発注せず処理全体を中断する。**
テキストのまま提出すると YP 側で「手動注文」のままになり、倉庫が 1688 を特定できない
（2026-09-07 に `Y0806-260907001` が該当し、YP から問い合わせが来た）。勝手に近いものを
選ぶと違う色・サイズが届くため、`SpecMatchRequiredError` を投げて止め、候補一覧を出して
ユーザーに聞く。仕入情報シートの「色・サイズ等指定」を直してから再実行する。
なお **仕様マッチ自体が出ない**場合（規格を持たない商品）は警告だけ出して発注を続ける。
